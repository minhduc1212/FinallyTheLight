...
| **Unified Trans & Glossary** | Dịch thô và trích xuất nhân vật/thuật ngữ/xưng hô chi tiết trong cùng 1 API call, đồng bộ hóa động sau mỗi chunk (không chia nhỏ API call). | Kích hoạt |
| **Editing & Proofreading** | Lượt API thứ hai tự động hiệu đính, biên tập và nâng cao chất lượng bản dịch thô mượt mà, có thể bật/tắt trong cấu hình. | Kích hoạt |
| **Detailed API Logging** | Tự động ghi nhận toàn bộ prompt, raw response, safety ratings, latency và thay đổi glossary chi tiết vào tệp log dự án. | Kích hoạt |
| **Visual Step Progress** | Hiển thị tiến trình dịch từng bước (Translating, Proofreading, Checking) trực tiếp trên terminal theo thời gian thực. | Kích hoạt |
...
│   └── genres.yaml          ← Phong cách dịch + term_categories theo thể loại
├── prompts/
│   └── system_prompts.yaml  ← System prompts, quy tắc nguồn và proofread templates
├── data/
│   └── {project}_glossary.json  ← Tệp thuật ngữ + nhân vật tự động đồng bộ
├── logs/
│   └── {project}.log        ← [MỚI] Tệp log chi tiết toàn bộ prompt, response, latency
...
│   ├── glossary_manager.py  ← Trình quản lý glossary thread-safe + logging sự kiện
│   └── pipeline.py          ← Điều phối viên chính (Dịch + Proofread + API Logger)
├── main.py                  ← CLI entry point
├── README.md                ← Hướng dẫn sử dụng nhanh
└── DOC.md                   ← Tài liệu chi tiết kiến trúc & mã nguồn
```
...
```yaml
features:
  auto_glossary: true                  # Tự động trích xuất nhân vật/thuật ngữ cùng lúc khi dịch
