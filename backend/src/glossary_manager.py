                ...
                if term and translation:
                    if term in self._data["terms"]:
                        ...
                        if existing != translation:
                            msg = f"[Glossary Conflict] Term '{term}': existing='{existing}', proposed='{translation}'. Keeping existing."
                            print(f"      ⚠️  {msg}")
                            self.logger.warning(msg)
                    else:
                        self._data["terms"][term] = translation
                        self.logger.info(f"[Glossary Add] Term: '{term}' -> '{translation}'")
                        added += 1
            if added:
                self.save()
            return added
                ...
                if name and info:
                    if name in self._data["characters"]:
                        ...
                        if merged != existing:
                            ...
                            self.logger.info(f"[Character Merge] '{name}': existing='{existing}', proposed='{info}', merged='{merged}'")
                            added += 1
                    else:
                        self._data["characters"][name] = info
                        self.logger.info(f"[Character Add] Name: '{name}' -> '{info}'")
                        added += 1
            if added:
                self.save()
            return added
